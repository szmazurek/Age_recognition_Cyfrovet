...
mean_vals = {"Red": 123, "Green": 135, "Blue": 146}

std_vals = {"Red": 68, "Green": 69, "Blue": 69}
        ...
        if display_heatmap or display_template:
            ...
            if display_template:
                ...
                if len(features):
                    ...
                    hm_sum = np.append(
                        hm_sum, heatmap.reshape((res[0], res[1], 3, 1)), axis=3
                    )
...
def hm_loader(path: string):
    ...
    template = template_pre.reshape(
        template_pre.shape[0], template_pre.shape[1] // 3, 3
    )
    return template
...
def img_rotate(
    src_image: np.ndarray, face_features: tuple, res: tuple = (IMG_SIZE, IMG_SIZE)
):
...
def img_preprocess(src: string, thrash: string):
    ...
    for filename in os.listdir(src):
        ...
        try:
            ...
            shutil.copy(
                string_e, wrong_address
            )  ## move incorrect files to other folder
            os.remove(string_e)
            continue
...
def metrics_calculator(input_img: np.ndarray, template_img: np.ndarray):
    ...
    """
    mse_metric = tf.convert_to_tensor(
        MSE(template_img.numpy().flatten(), input_img.numpy().flatten())
    )
    ssim_metric = tf.image.ssim(
        input_img, template_img, max_val=max_val_ssim(input_img, template_img)
    ).numpy()[0]
